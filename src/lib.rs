...
static mut SECONDS_LEFT: u64 = 180;
...
pub fn start_game(timed: bool) -> Result<(), JsValue> {	
	...
	toggle_element(&document, "skipped-problems", false)?;
	...
	let time = Closure::wrap(Box::new(move || unsafe {
		SECONDS_LEFT -= 1;
...
pub fn run() -> Result<(), JsValue> {
	...
	let button = document.get_element_by_id("start-button-timed").unwrap()
		.dyn_into::<web_sys::HtmlButtonElement>().unwrap()
		.set_onclick(Some(start.as_ref().unchecked_ref()));
	let skip = document.get_element_by_id("skip-button").unwrap()
		.dyn_into::<web_sys::HtmlButtonElement>().unwrap()
	...
	// input.set_onchange(Some(validate.as_ref().unchecked_ref())); 
	// input.set_onpaste(Some(validate.as_ref().unchecked_ref()));
